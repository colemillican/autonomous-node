...
# ----------------------------
PI_IP = get_env_str("PI_IP", "192.168.1.104")
PORT = get_env_int("PORT", 5000, min_value=1, max_value=65535)
...
WINDOW_NAME = get_env_str("WINDOW_NAME", "Smart Security Detection v2")
    ...
    def _reader_loop(self):
        ...
        while self.running:
            ...
            if cap is None:
                ...
                if new_cap is None:
                    logging.warning("Stream open failed. Retrying in %.2fs", backoff)
                    time.sleep(backoff)
                    backoff = min(backoff * 2, RECONNECT_BACKOFF_MAX_SEC)
                    continue
                with self.cap_lock:
                    self.cap = new_cap
                logging.info("Connected to stream: %s", self.stream_url)
                backoff = 0.1
                read_fail_count = 0
                continue
            ...
            if not ret:
                ...
                if read_fail_count >= READ_FAIL_REOPEN_THRESHOLD:
                    logging.warning("Too many read failures (%d). Reopening stream...", read_fail_count)
                    with self.cap_lock:
                        if self.cap is cap:
                            self.cap.release()
                            self.cap = None
...
class AlertWorker:
    ...
    def _send_alert(self, frame, message):
        ...
        # Optional local snapshot for debugging/forensics
        try:
...
def main():
    ...
    try:
        while not stop_event.is_set():
            frame_id, frame = stream.get_latest()
            if frame is None:
                time.sleep(0.01)
                continue

            # Skip duplicate frame references to avoid wasted compute.
